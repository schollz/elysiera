...
pitch_mod_hz  = group_mod(vslider("[5]Pitch Mod Rate [style:knob] [unit:hz]", 3.1, 0.01, 10.0, 0.01));
pitch_mod_mix = group_mod(vslider("[6]Pitch Mod Mix [style:knob] [unit:%]", 1.0, 0.0, 1.0, 0.01));
mod_hz        = group_mod(vslider("[7]Volume Mod Rate [style:knob] [unit:hz]", 3.1, 0.01, 10.0, 0.01));
mod_mix       = group_mod(vslider("[8]Volume Mod Mix [style:knob] [unit:%]", 1.0, 0.0, 1.0, 0.01));
...
process = _, _ :
    ...
    : ef.dryWetMixerConstantPower(mix, (_ , _
        ...
        : (
            ...
            (_ <:
                ef.transpose(c_samples, c_xfade, pitch_a) * (os.oscsin(pitch_mod_hz) * pitch_mod_mix + (1 - pitch_mod_mix)) * pitch_a_vol
                + ef.transpose(c_samples, c_xfade, pitch_b) * (os.osccos(pitch_mod_hz) * pitch_mod_mix + (1 - pitch_mod_mix)) * pitch_b_vol
            ),
            (_ <:
                ef.transpose(c_samples, c_xfade, pitch_a) * (os.osccos(pitch_mod_hz) * pitch_mod_mix + (1 - pitch_mod_mix)) * pitch_a_vol
                + ef.transpose(c_samples, c_xfade, pitch_b) * (os.oscsin(pitch_mod_hz) * pitch_mod_mix + (1 - pitch_mod_mix)) * pitch_b_vol
            )
        )
        // Modulate volume
        : ef.dryWetMixerConstantPower(mod_mix, (*(os.oscsin(mod_hz)), *(os.osccos(mod_hz))))
        // Reverb
